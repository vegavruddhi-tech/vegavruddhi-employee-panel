...
export default function Navbar({ emp, taskCount, token }) {
  ...
  const refreshMeetingCount = useCallback(() => {
    if (!token || !emp?.email) return;
    fetch(`${API_BASE}/api/meetings/my-meetings?email=${encodeURIComponent(emp.email)}`, {
      headers: { Authorization: 'Bearer ' + token }
    })
      .then(r => r.json())
      .then(data => {
        if (data.success) setMeetingCount(data.unreadCount || 0);
      })
      .catch(() => {});
  }, [token, emp?.email]);
  ...
  useEffect(() => {
    if (!token || !emp?.email) return;
    refreshMeetingCount();
    const interval = setInterval(refreshMeetingCount, 30000); // Check every 30 seconds
    return () => clearInterval(interval);
  }, [token, emp?.email, refreshMeetingCount]);
